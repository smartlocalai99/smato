...
export function createDriverApi(client) {
  return {
    ...
    // Powers the "pick a tablet" suggestions on the driver form — every
    // known auto number, so registering a driver can link straight to one.
    async listAutos() {
      const { data, error } = await client.from("autos").select("auto_number").order("auto_number");
      if (error) throw error;
      return data || [];
    },
    async markPaid(id) {
