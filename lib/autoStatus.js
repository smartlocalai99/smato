...
// Tablets phone home every 3 minutes (HEARTBEAT_INTERVAL_MS in the player),
// so these windows need enough slack for one missed beat before downgrading.
...
// Charging counts as "fine" regardless of level — it's on its way up, not
// stranded. A tablet plugged in all day should never flag red just because
// it dipped low once before charging kicked in.
export function batteryStyle(level, charging) {
  if (level == null) return "text-text-faint";
...
export function batteryLabel(level, charging) {
